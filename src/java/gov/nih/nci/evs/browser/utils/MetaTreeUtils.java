...
//KLO
    ...
    //public void addChildren(TreeItem ti, String scheme, CodingSchemeVersionOrTag csvt,
    public TreeItem addChildren(TreeItem ti, String scheme, CodingSchemeVersionOrTag csvt,
            String sab, String code, Set<String> codesToExclude,
...
//KLO
 ...
 String root_name = getAtomName(rcr.getReferencedEntry(), NCI_SOURCE, "PT");
								...
								if (sabMatch.length > 0) {
									...
									if (!visited_links.contains(link)) {
										...
										if (tiParent == null) {
											...
											tiParent = addChildren(tiParent, scheme, csvt, sab, parentCode, code2Tree.keySet());
...
/*
        // testing
		par_chd_assoc_list = new ArrayList();
		par_chd_assoc_list.add("CHD");
		try {
			mbs = (MetaBrowserService)lbSvc.getGenericExtension("metabrowser-extension");
			map = mbs.getBySourceTabDisplay(ti.code, sab, par_chd_assoc_list, Direction.TARGETOF);
		...
		cui2SynonymsMap = createCUI2SynonymsHahMap(map);
		//ti.expandable = false;
		keyset = cui2SynonymsMap.keySet();
