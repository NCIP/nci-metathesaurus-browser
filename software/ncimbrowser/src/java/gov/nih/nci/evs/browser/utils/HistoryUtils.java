...
public class HistoryUtils {
     ...
     * lbSvc.
     * getHistoryService(codingSchemeName);//Constants.CODING_SCHEME_NAME);
     * return getEditActions(lbSvc, hs, codingSchemeName, vers, ltag, code); }
     * 
     * public static Vector<String> getEditActions(LexBIGService lbSvc,
     * HistoryService hs, String codingSchemeName, String vers, String ltag,
     * String code) throws LBException { //LexBIGService lbSvc =
     * RemoteServerUtil.createLexBIGService(); //HistoryService hs =
     * lbSvc.getHistoryService
     * (codingSchemeName);//Constants.CODING_SCHEME_NAME);
     * 
     * try { NCIChangeEventList list = hs.getEditActionList(Constructors
     * .createConceptReference(code, null), null, null); return
     * getEditActions(lbSvc, codingSchemeName, vers, ltag, code, list); } catch
     ...
     * Vector<String> v = new Vector<String>(); HashSet<String> hset = new
     * HashSet<String>(); while (enumeration.hasMoreElements()) { NCIChangeEvent
     * event = enumeration.nextElement(); ChangeType type =
     * event.getEditaction(); Date date = event.getEditDate();
     * 
     * //_logger.debug(" HistoryUtils date: " + date);
     * 
     * String rCode = event.getReferencecode(); String desc = "N/A"; if (rCode
     * != null && rCode.length() > 0 && ! rCode.equalsIgnoreCase("null")) {
     * Concept c = getConceptByCode( codingSchemeName, vers, ltag, rCode); //KLO
     * if (c != null) { String name = c.getEntityDescription().getContent();
     * desc = name + " (Code " + rCode + ")"; } else { desc = rCode; } }
     * 
     * String info = type + "|" + _dataFormatter.format(date) + "|" + desc; if
     * (hset.contains(info)) continue;
     * 
     * //_logger.debug("NCIChangeEvent: " + info); v.add(info); hset.add(info);
     * } return v; }
     * 
     * private static Vector<String> getEditActions(LexBIGService lbSvc, String
     * codingSchemeName, String vers, String ltag, String code,
     * NCIChangeEventList list) { Enumeration<NCIChangeEvent> enumeration =
     * list.enumerateEntry(); Vector<String> v = new Vector<String>();
     * HashSet<String> hset = new HashSet<String>(); while
     * (enumeration.hasMoreElements()) { NCIChangeEvent event =
     * enumeration.nextElement(); ChangeType type = event.getEditaction(); Date
     * date = event.getEditDate();
     * 
     * String rCode = event.getReferencecode(); String desc = "N/A"; if (rCode
     * != null && rCode.length() > 0 && ! rCode.equalsIgnoreCase("null")) {
     * Concept c = getConceptByCode(lbSvc, codingSchemeName, vers, ltag, rCode);
     * //KLO if (c != null) { String name =
     * c.getEntityDescription().getContent(); desc = name + " (Code " + rCode +
     * ")"; } else { desc = rCode; } }
     * 
     * String info = type + "|" + _dataFormatter.format(date) + "|" + desc; if
     * (hset.contains(info)) continue;
     ...
     * 
     * 
     * public static ConceptReferenceList createConceptReferenceList(String[]
     * codes, String codingSchemeName) { if (codes == null) { return null; }
     * ConceptReferenceList list = new ConceptReferenceList(); for (int i = 0; i
     * < codes.length; i++) { ConceptReference cr = new ConceptReference();
     * cr.setCodingSchemeName(codingSchemeName); cr.setConceptCode(codes[i]);
     * list.addConceptReference(cr); } return list; }
     * 
     * public static Concept getConceptByCode(LexBIGService lbSvc, String
     * codingSchemeName, String vers, String ltag, String code) { try { if
     * (lbSvc == null) { _logger.warn("lbSvc == null???"); return null; }
     * CodingSchemeVersionOrTag versionOrTag = new CodingSchemeVersionOrTag();
     * versionOrTag.setVersion(vers);
     * 
     * ConceptReferenceList crefs = createConceptReferenceList( new String[]
     * {code}, codingSchemeName);
     * 
     * CodedNodeSet cns = null; try { cns =
     * lbSvc.getCodingSchemeConcepts(codingSchemeName, versionOrTag); cns =
     * cns.restrictToCodes(crefs); ResolvedConceptReferenceList matches =
     * cns.resolveToList(null, null, null, 1); if (matches == null) {
     * _logger.warn("Concept not found."); return null; } int count =
     * matches.getResolvedConceptReferenceCount(); // Analyze the result ... if
     * (count == 0) return null; if (count > 0) { try { ResolvedConceptReference
     * ref = (ResolvedConceptReference)
     * matches.enumerateResolvedConceptReference().nextElement(); Concept entry
     * = ref.getReferencedEntry(); return entry; } catch (Exception ex1) {
     * _logger.error("Exception entry == null"); return null; } } } catch
     * (Exception e1) { e1.printStackTrace(); return null; } } catch (Exception
     * e) { e.printStackTrace(); return null; } return null; }
     * 
     * 
     * public static Concept getConceptByCode(String codingSchemeName, String
     * vers, String ltag, String code) { try { LexBIGService lbSvc = new
     * RemoteServerUtil().createLexBIGService(); return getConceptByCode(lbSvc,
     * codingSchemeName, vers, ltag, code); } catch (Exception e) {
     * e.printStackTrace(); return null; } }
     * 
     * 
     * public static Vector<String> getAncestors(String codingSchemeName, String
     * vers, String ltag, String code) throws LBException { LexBIGService lbSvc
     * = RemoteServerUtil.createLexBIGService(); HistoryService hs =
     * lbSvc.getHistoryService
     * (codingSchemeName);//Constants.CODING_SCHEME_NAME);
     * 
     * try { NCIChangeEventList list = hs.getAncestors(Constructors
     ...
     * 
     * 
     * public static void main(String[] args) { try { HistoryUtils test = new
     * HistoryUtils();
     * 
     * String scheme = "NCI Metathesaurus"; String version = null; String code =
     * "C0260526";
     * 
     * code = "C1325880";
     * 
     ...
     * _logger.info(v.size());
     * 
     * _logger.info("\n\nTesting getAncestors ..."); v =
     * test.getAncestors(scheme, version, null, code); _logger.info(v.size());
     * 
     * _logger.info("\n\nTesting getDescendants ..."); v =
     * test.getDescendants(scheme, version, null, code); _logger.info(v.size());
     * 
    ...
    private static Vector<String> getEditActions(LexBIGService lbSvc,
        ...
        try {
             ...
             * _logger.debug("getConceptByCode codingSchemeName: " +
             * codingSchemeName); _logger.debug("getConceptByCode vers: " +
             * vers); _logger.debug("getConceptByCode ltag: " + ltag);
             ...
             * null), null, null); _logger.debug("calling getEditActions... ");
    ...
    public static String getReferencedCUI(String code) {
        ...
        try {
            Vector<String> v =
                getEditActions(Constants.CODING_SCHEME_NAME, null, null, code);
            if (v != null) {
                if (v.size() == 0)
                    _logger
